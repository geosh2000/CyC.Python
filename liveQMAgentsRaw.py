...
for item in agents:
    ...
    for field in item.keys():
        ...
        if field == 'Freesincepauorcalltst':
            dupQ += "%s=IF(Freesincepauorcalltst>VALUES(%s),Freesincepauorcalltst,VALUES(%s))," % (field, field, field)
        else:
...
updateFlag = "DELETE FROM liveMonitor WHERE updateFlag = 0 AND Agent NOT LIKE 'wait%'"
cur.execute( updateFlag )
db.commit()
print "Done!"
cur.close()
